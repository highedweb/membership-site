...
function memberimport_page() {
	...
	if ($valid_nonce && array_key_exists( 'mode', $_POST ) && $_POST['mode'] == 'submit' ) {
		$arr_rows = file( $_FILES['csv_file']['tmp_name'] );
		$login_username = isset( $_POST['login_username'] ) ? sanitize_user( $_POST['login_username'] ) : false;
		$login_username = ( 1 == $login_username ) ? true : false;

		$new_member_notification = isset( $_POST['new_member_notification'] ) ? sanitize_text_field( $_POST['new_member_notification'] ) : false;
		$new_member_notification = ( 1 == $new_member_notification ) ? true : false;
		...
		if ( is_array( $arr_rows ) ) {
			...
			foreach ( $arr_rows as $row ) {
				...
				if ( ( $login_username ) && ( ( '' == $userdata['user_email'] ) || ( ! is_email( $userdata['user_email'] ) ) ) )
