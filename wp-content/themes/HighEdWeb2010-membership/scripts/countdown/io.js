...
var totalDotCount = 0, dotThreshold = 5; //was 150
var iterations = 1, timeStep = 1 / 20, ground;  //was timeStep = 1 / 20
...
function Digit(opts) {
  ...
  this.num = opts.num;
  this.blankColor = opts.blankColor || '#f7f7fa';
  ...
  for (var i = 0; i < this.matrix.length; i++) {
      for (var j = 0; j < this.matrix[i].length; j++) {
          this.dots.push(new Dot({
              x: this.x + 7*j,  //was this.x + 19*j
              y: this.y + 7*i,  //this.y + 19*i
              ctx: this.ctx,
...
(function() {
      ...
      for (var i = 0; i < digits.length; i++) {
          ...
          if (oldDigits.length > 0 && digits[i] == oldDigits[i].num) {
              ...
              /**
                ...
                */
              if (digits[i] == ':') {
                  cursorX += (2*10);  //was (2*18)
              } else {
                  cursorX += (5*7)-1; //(5*19)-1
              }
          } else {
  ...
  /**
    ...
    */
  var countdownTo = new Date(2011, 09, 23, 0, 0, 10).getTime();  //it's a month off; don't know why
